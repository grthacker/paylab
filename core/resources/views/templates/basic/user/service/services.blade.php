...
@section('content')
                    ...
                    <div class="col-lg-4 col-md-6 mb-30">
                        <div class="d-widget">
                            ...
                            <div class="mt-4 mb-4 text-center">
                                <ul>
                                    ...
                                    <li>
                                        ...
                                        <span class="">{{ getAmount($data->fixed_charge) }}
                            ...
                            {{-- <div class="price-footer">
                                <a href="javascript:void(0)" class="cmn-btn bg-primary w-100 text-center apply"
                                    data-toggle="modal" data-target="#applyModal" data-id="{{ $data->id }}"
                            ...
                            <div class="price-footer">
                                <a href="{{ route('user.service.confirm', ['id' => $data->id]) }}"
                                    class="cmn-btn bg-primary w-100 text-center "
                                    data-fix_charge="{{ getAmount($data->fixed_charge) }}"
                                    data-percent_charge="{{ getAmount($data->percent_charge) }}"
                                    data-category="{{ __($data->category->name) }}"
                                    data-service="{{ __($data->name) }}">@lang('Continue')</a>
    ...
    {{-- <div class="modal fade" id="applyModal" tabindex="-1" role="dialog" aria-labelledby="depositModalLabel"
        aria-hidden="true">
            ...
            <div class="modal-content">
                ...
                <form action="{{ route('user.service.apply') }}" method="post">
                    ...
                    <div class="modal-body">
                        ...
                        <div class="form-group">
                            <label for="operator" style="font-weight: bold; color:black;">@lang('Plans'):</label>
                            <select class="form-control form-control-lg" id="operator" name="operator" required="">
                                <option value="" selected disabled>@lang('Select Operator')</option>
                                @foreach ($plans['plans'] as $plan)
                                    <option value="{{ '$plan' }}">
                                        {{ 'Rs:' . $plan['planAmount'] }} |
                                        @if (isset($plan['dataBenefits']))
                                            {{ 'Data:' . $plan['dataBenefits'] }} |
                                        @endif
                        ...
                        <div class="form-group">
                            ...
                            <div class="input-group">
                                <input id="amount" type="text" class="form-control form-control-lg"
                                    ...
                                    name="amount" placeholder="0.00" required="" value="{{ old('amount') }}" readonly>
                                <div class="input-group-prepend">
                                    <span
                                        class="input-group-text currency-addon addon-bg bg-primary text-white">{{ __($general->cur_text) }}</span>
                                </div>
                            </div>
                        </div>
                    </div>
...
@push('script')
    ...
    <script>
        document.getElementById('operator').addEventListener('change', function() {
            ...
            var rsPosition = selectedOption.indexOf('Rs:');
            var dataPosition = selectedOption.indexOf('|');
            if (rsPosition !== -1 && dataPosition !== -1) {
                var amountValue = selectedOption.substring(rsPosition + 3, dataPosition).trim();
